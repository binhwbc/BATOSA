...
class ControllerDesignMenu extends Controller {
	...
	public function index($setting) {
		...
		$positions = $this->model_design_menu->getPosition($setting);


		if ($positions) {
			$results = $this->model_design_menu->getMenuByPosition($positions['position_id']);
		} else{
			$results = '';
		}
		...
		if($results){
			...
			return $this->load->view('design/menu', $data);
		}
			...
			foreach($query_group as $key_group => $value_group){
				if($menu_group_id == $value_group['menu_id']){
					foreach($query_group as $key_group => $value_group){
						...
						$menu_group_create[$value_group['menu_group_id']]['thumb'] = $this->model_tool_image->resize($value_group['image'], 300,450);
					}
				}
			}
			return $menu_group_create;
